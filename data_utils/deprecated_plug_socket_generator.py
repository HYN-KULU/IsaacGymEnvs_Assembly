...
def create_plug_with_grasp_post(
    ...
    # ---------------------------------------------------------
    # 【核心修正】强制回正 X/Y 轴，确保 root_pos 就在几何中心
    # ---------------------------------------------------------
    # 获取当前网格的包围盒
    bounds = plug_mesh.bounds  # [[min_x, min_y, min_z], [max_x, max_y, max_z]]
    center_x = (bounds[0][0] + bounds[1][0]) / 2.0
    center_y = (bounds[0][1] + bounds[1][1]) / 2.0
    
    # 平移网格：让 X/Y 回到 0，但 Z 保持不动（底面依然在 Z=0）
    plug_mesh.apply_translation([-center_x, -center_y, 0.0])
    # ---------------------------------------------------------
    ...
    meta = {
        ...
        "center_offset_x": float(center_x), # 记录偏移供调试
        "center_offset_y": float(center_y)
    }
...
def create_socket_from_profile(
    ...
    mesh2d, outer_segments, inner_segments, _, _ = build_annulus_2d_mesh_triangle(
        outer_profile=outer_profile,
        inner_profile=hole_profile,
        max_area=triangle_max_area,
    )
    ...
    # ==========================================================
    # 【核心修复】计算孔洞中心，并将整个 Socket Mesh 平移对齐到 (0,0)
    # ==========================================================
    # 使用 hole_profile 的包围盒中心作为对齐基准
    h_minx, h_miny, h_maxx, h_maxy = hole_profile.bounds
    offset_x = (h_minx + h_maxx) / 2.0
    offset_y = (h_miny + h_maxy) / 2.0

    # 将网格在 X/Y 平面上回正，Z 轴保持不变（0 到 socket_height）
    socket_mesh.apply_translation([-offset_x, -offset_y, 0.0])
    
    # 同时也平移 hole_profile，确保返回的 profile 与 mesh 空间位置一致
    from shapely.affinity import translate
    hole_profile_aligned = translate(hole_profile, xoff=-offset_x, yoff=-offset_y)
    # ==========================================================

    socket_mesh.remove_duplicate_faces()
    ...
    # socket_profile 仅用于可视化，这里也做下简单的平移对齐
    socket_profile = get_main_polygon(fix_geom(outer_profile.difference(hole_profile)))
    socket_profile_aligned = translate(socket_profile, xoff=-offset_x, yoff=-offset_y)

    return socket_mesh, hole_profile_aligned, socket_profile_aligned, meta
...
def generate_grasp(plug_total_height=0.09, asset_id=""):
    x = np.random.uniform(-0.0015, 0.0015)
    y = np.random.uniform(-0.0015, 0.0015)
    z = plug_total_height + np.random.uniform(0.07, 0.11)

    qx = 0.0
    qy = 1.0
    qz = np.random.uniform(-0.08, 0.08)
    qw = 0.0
...
def save_asset_bundle(
    ...
    grasp = generate_grasp(plug_total_height=plug_meta["plug_total_height"], asset_id=asset_id)
    ...
    # 在 save_asset_bundle 函数里，mesh.export 之前加入：
...
if __name__ == "__main__":
    ...
    result = save_asset_bundle(
        ...
        # socket_smooth_radius=0.00030,
        # socket_simplify_tol=0.00008,
        socket_smooth_radius=0,
        socket_simplify_tol=0,
        trim_extra_height=0.003,
        triangle_max_area=None,
        seed=42,
    )
