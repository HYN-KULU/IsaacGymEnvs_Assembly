...
class DepthActionDataset(Dataset):
    def __init__(self, hdf5_file, transform=None, normalize=True, scale_to_unit=True, obs_dir="./data"):
        """
        Args:
    ...
    def __getitem__(self, idx):
        # depth (H, W) -> (1, H, W)
        rgb = np.load(f"{self.obs_dir}/rgb_{idx}.npy").astype(np.float32)
        ...
        if self.normalize:
            ...
            actions = np.concatenate([norm_delta, rot6d], axis=-1)
