...
from dataset.flow_diffusion_policy_multitask import DepthActionDataset
...
def train(args_override):
    ...
    dataset = DepthActionDataset(hdf5_file= args.data_path, index_path=args.index_path)
    ...
    for epoch in range(args.resume_epoch + 1, args.num_epochs):
        ...
        for batch in pbar:
            ...
            flow = batch["flow"].to(device)                 # (B, 2, H, W)
            loss = policy(depth=depth, proprioception=proprio, actions=actions, flow = flow)
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
            lr_scheduler.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--index_path', type=str, default=None, required=True, help="Path to index.npy")
