...
from policy.force_diffusion_policy_image_condition import Diffusion_Policy
from diffusion_utils.transformation import rot_trans_mat, apply_mat_to_pose, apply_mat_to_pcd, xyz_rot_transform
from data_utils.socket_image import save_depth_visualization, rotate_socket_image_opposite_gripper,relative_yaw_from_quats, quat_xyzw_to_yaw
...
def run_env(cfg: DictConfig):
    ...
    for trial in range(5):
        ...
        for t in range(100):
            ...
            fingertip_centered_quat=envs.fingertip_centered_quat.clone()
            # Rotating the socket's depth image
            socket_depth_list = []
            for i in range(12):
                q_init = init_fingertip_centered_quat_list[i]
                q_final = fingertip_centered_quat[i].cpu().numpy()
                socket_depth, _ = rotate_socket_image_opposite_gripper(init_socket_depth_list[i], q_init, q_final)
