...
def run_env(cfg: DictConfig):
    ...
    for _ in range(26):
        ...
        pos_min=torch.tensor([-0.0473, -0.0427,  0.4595], device='cuda:0')
        pos_max=torch.tensor([0.0456, 0.0491, 0.5995], device='cuda:0')
        ...
        # selected_last_images = [images[i] for i in env_ids.cpu().tolist()]
    ...
    np.save("error_pos_gripper.npy",np.concatenate(error_pos_list,axis=0))
    np.save("error_quat_gripper.npy",np.concatenate(error_quat_list,axis=0))
    os._exit(0)
