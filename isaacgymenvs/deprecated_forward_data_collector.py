...
def quat_slerp_step(current_q, target_q, alpha=0.05):
    """
    Move current_q a small fraction alpha toward target_q.
    alpha in [0,1]
    """
    q_err = quat_error(current_q, target_q)
    axis, angle = quat_to_axis_angle(q_err)
    q_step = axis_angle_to_quat(axis, angle * alpha)
    return quat_normalize(quat_mul(q_step, current_q))
...
def run_env(cfg: DictConfig):
    ...
    action_list=[]
    depth_list=[]
    if cfg.seed in [0,1,2,3,4,5,6]:
        recovery = False
    else:
        recovery = True
    steps = 150 if recovery else 200
    disturb_steps = 50 if cfg.seed in [7,8,9] else 100
    ###
    recovery = False
    disturb_steps = 0
    steps = 250
    ...
    # for i in range(200):
        ...
        if i > 50:
            init_yaw = yaw_from_quat(init_plug_quat)
            curr_yaw = yaw_from_quat(envs.plug_quat)
            yaw_error = wrap_to_pi(init_yaw - curr_yaw)

            print("init_yaw_deg:", torch.rad2deg(init_yaw))
            print("curr_yaw_deg:", torch.rad2deg(curr_yaw))
            print("yaw_error_deg:", torch.rad2deg(yaw_error))

            # apply only a small step each iteration
            max_yaw_step = torch.deg2rad(torch.tensor(0.5, device=envs.device))
            yaw_step = torch.clamp(yaw_error, min=-max_yaw_step, max=max_yaw_step)

            delta_yaw_quat = yaw_quat(yaw_step)
            next_tgt_quat = quat_mul(delta_yaw_quat, envs.fingertip_centered_quat.clone())
        ...
        force_traj.append(envs.vec_sensor_tensor.view(-1,3,6).clone())
        print(envs.vec_sensor_tensor.view(-1,3,6).clone())
    ...
    # if cfg.seed == 0:
    print(init_plug_pos - envs.plug_pos)
    for env_id in range(num_envs):
            envs.save_first_env_images(out_dir=f"rollout_orient/{envs.cfg_task.env.desired_subassemblies[0]}", index=env_id, reverse=False) 
    # import pdb;pdb.set_trace()
    # envs._save_log_traj(action_array_padded[:,envs.success_env_ids,:], force_array[:,envs.success_env_ids,:,:])  
    import pdb;pdb.set_trace()
    os._exit(0)
