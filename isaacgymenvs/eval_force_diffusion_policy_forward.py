...
from policy.force_diffusion_policy import Diffusion_Policy
...
def run_env(cfg: DictConfig):
    ...
    data=load_processed_dataset("/home/ubuntu/automate/IsaacGymEnvs_Assembly/processed_dataset_forward_0413.h5")
    # data=load_processed_dataset("/home/ubuntu/automate/IsaacGymEnvs_Assembly/processed_dataset_forward_0412.h5")
    ...
    ### Load Policy
    # ckpt_path = "/home/ubuntu/automate/IsaacGymEnvs_Assembly/logs/automate/force_diffusion_policy_zeroforce/policy_epoch_110.ckpt"  # or policy_last.ckpt
    ckpt_path = "/home/ubuntu/automate/IsaacGymEnvs_Assembly/logs/automate/force_diffusion_policy_multitask/policy_epoch_48.ckpt"  # or policy_last.ckpt
    ...
    for trial in range(1):
        ...
        for t in range(50):
            ...
            current_force_np = envs.vec_sensor_tensor.view(-1, 3, 6).detach().cpu().numpy()[:,0,:3]
            delta_force = current_force_np - init_force_np
            camera3_vinv = np.array(camera3_vinv_list)
            R_wc = camera3_vinv[:,:3,:3]
            for force_id in range(12):
                f_world = delta_force[force_id]
                f_vec_cam = R_wc[force_id] @ f_world
                delta_force[force_id] = f_vec_cam
            # delta_force[...] = 0
            force_normalized = ((delta_force - force_min) / (force_max - force_min + 1e-8)) * 2.0 -1.0
            ...
            raw_actions=policy(depth_rot.unsqueeze(dim=1)[...,::rate,::rate][...,30:-30,40:-40],proprioception[...,2:],actions=None, force = torch.from_numpy(force_normalized).cuda())
        ...
        for env_id in range(12):
            envs.save_first_env_images(out_dir=f"rollout_eval_forward_110ckpt/{envs.cfg_task.env.desired_subassemblies[0]}/{trial}", index=env_id, reverse=False)
